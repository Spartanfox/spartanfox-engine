 ...
 */
package com.spartanfox.engine.utils;
...
public class BodyEditorLoader {
	...
	public void attachFixture(Body body, String name, FixtureDef fd, float scale) {
		...
		if (rbModel == null) throw new RuntimeException("Name '" + name + "' was not found.");
                System.out.println("scaling model");
		Vector2 origin = vec.set(rbModel.origin).scl(scale);
                
                System.out.println("creating polygonsses" );
		...
		for (int i=0, n=rbModel.polygons.size(); i<n; i++) {
			...
			for (int ii=0, nn=vertices.length; ii<nn; ii++) {
				...
				vertices[ii].sub(origin);
                        ...
                        System.out.println("setting polgyonshape");
			polygonShape.set(vertices);
			fd.shape = polygonShape;
                        System.out.println("adding fixture to body");
                        System.out.println(fd.shape.getRadius());
